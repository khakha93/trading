...
def render_analysis_tab(vol_change_val, rate_val) -> None:
    if not render_basket_overview():
        return
    ...
    render_analysis_chart_and_summary(vol_change_val, rate_val, days_to_expiry_val, target_underlying_val)


def render_search_tab(vol_change_val, rate_val) -> None:
    render_search_tab_ui(vol_change_val, rate_val)


def render_app() -> None:
    configure_app()

    vol_change_val, rate_val = render_sidebar()
    render_app_header()

    tab2, tab1 = st.tabs(["🔍 종목 검색 & 추가", "📊 포트폴리오 분석 & 시뮬레이션"])

    with tab1:
        render_analysis_tab(vol_change_val, rate_val)

    with tab2:
        render_search_tab(vol_change_val, rate_val)
